...
import { Filter, Trash2, Edit2, Save, X } from 'lucide-react';
...
export default function Penjualan({ penjualan, shift, onUpdateShift }) {
  ...
  const jenisTehList = useMemo(() => {
    const types = new Set();
    penjualan.forEach(t => t.items?.forEach(item => { if (item.nama) types.add(item.nama); }));
  ...
  const filteredPenjualan = useMemo(() => {
    return penjualan.filter(t => {
      ...
      if (filterJenisTeh !== 'SEMUA' && !t.items?.some(item => item.nama === filterJenisTeh)) return false;
      return true;
    });
  ...
  const handleOpenEdit = (transaksi) => {
    ...
    // Salin item transaksi agar bisa dimanipulasi di form lokal modal
  ...
  const handleSaveEdit = () => {
    // 1. Hitung total cups dan susu awal (sebelum diedit) untuk kalkulasi selisih stok balik
    ...
    // 2. Hitung total harga baru, total cups baru, dan kebutuhan susu baru dari form edit
    ...
    // 3. Validasi batas ketersediaan sisa stok cup & susu di toko
    ...
    // 4. Petakan struktur transaksi baru ke dalam database log shift lokal
    const updatedTransaksi = shift.transaksi.map(t => {
      if (t.id === editingTransaksi.id) {
        return {
          ...
          metode: editMetode,
          total: totalBaru,
          items: editItems.filter(item => item.qty > 0) // Buang item jika qty diset ke 0
        };
      }
      return t;
    });
    ...
    const updatedShift = {
      ...
      stokDasar: {
        ...
        susu: Math.max(0, stokSusuSekarang => (stokSusuTersedia - selisihSusu))
      },
      transaksi: updatedTransaksi
    };
              ...
              <tr key={t.id || t.waktu}>
                ...
                <td>
                  ...
                  <button 
                    ...
                    style={{ background: 'none', border: 'none', color: '#007BFF', cursor: 'pointer', marginRight: 12 }}
                  >
                    <Edit2 size={16} />
                  </button>
        ...
        <div className="modal-overlay" onClick={() => setOpenEditModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '450px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
              ...
              <button onClick={() => setOpenEditModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                <X size={20} />
              </button>
            </div>
                  ...
                  <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, gap: 10 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <input 
                        type="number" 
                        min="0"
                        style={{ width: '60px', padding: '4px', textAlignment: 'center' }}
                        value={item.qty} 
